...
import { getQuickActions } from "@/libs/constants/assistantConstants";
...
const ChatTextArea = () => {
  ...
  const handleSend = async () => {
    ...
    // 2. Call backend
    const response = await sendToBackend(userMsg, messages, assistantId);
...
const QuickActionsDropUp = ({
  ...
  const messages = useMessagesStore((state) => state.messages);
  const refineMessage = useMessagesStore((state) => state.refineMessage);
  const addMessage = useMessagesStore((state) => state.addMessage);

  const lastAssistantMessage = [...messages]
    .reverse()
    .find((msg) => msg.role === "assistant");
  const originalText = lastAssistantMessage?.content || "";

  const quickActions = getQuickActions(originalText, addMessage, refineMessage);
  return (
    <motion.div
      ref={quickActionsRef}
      ...
      <div className="flex items-center justify-between px-3 mb-2">
        ...
        <div className="relative group">
          ...
          <div className="absolute bottom-full mb-1 right-0 w-48 bg-black text-white text-xs p-2 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
            These actions apply to the{" "}
            <strong>most recent email generated by the assistant.</strong>.
          </div>
        </div>
      </div>
